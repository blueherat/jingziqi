    ...
    def policy_move(self, state, current_player, size=3):
        ...
        for empty_position in empty_positions:
            ...
            if key in self.table: # 在v值表中
                black_reward, white_reward, draw = self.table[key]
...
class Game(object):
    ...
    def selfplay(self):
        ...
        for one in self.episode:
            ...
            if done:
                self.agent.table.setdefault(state, np.array([0, 0, 0]))
                self.agent.table[state] = reward
                # print(self.agent.table[state])
            if pre_state is not None:
                self.agent.table.setdefault(state, np.array([0, 0, 0]))
                self.agent.table.setdefault(pre_state, np.array([0, 0, 0]))
                # print("state: ",self.agent.table[state])
                # print("pre_state", self.agent.table[pre_state])
                td_target = 0 + self.gamma * self.agent.table[state]
                # print("td_taget: ", td_target)
                self.agent.table[pre_state] = self.agent.table[pre_state] + self.alpha * (td_target - self.agent.table[pre_state])
                # print(self.agent.table[pre_state])
    ...
    def load_table(self, path):
        with open(path, 'rb') as file:
            self.agent.table = joblib.load(file)
